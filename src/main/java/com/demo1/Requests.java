...
public class Requests {
    ...
    public static void updateStorageAfterAssembly(Client client, String detailName, int detailCount) {
        ...
        if (newQuantity == 0) {
            Requests.deleteDetailByNameRequest(client, detail.getDetailName());
        } else {
            Requests.updateDetailRequest(client, detail, newQuantity);
        }
    ...
    public static void updateDetailRequest(Client client, Detail detail, int quantity) {
        Detail updateDetail = new Detail(detail.getDetailName(),quantity);
    ...
    public static Response universalGetRequest(Client client) {
        return client.target(SERVER_URL + "/details")
                .request(MediaType.APPLICATION_JSON)
                .get();
    }
    ...
    public static Response getShelfByIdRequest(Client client,Long id) {
        return client.target(SERVER_URL + "/shelf/" + id)
                .request(MediaType.APPLICATION_JSON)
                .get();
    }
